我现在在进行一项`HSTU`模型魔改任务，具体来说，我希望向HSTU模型中引入稀疏注意力模块，以实现精度与性能双提升。
你的任务是基于我给出的魔改后HSTU模型（命名为HSTU_bsa），以及参考我提供的两份参考实现，正确地用triton实现HSTU_bsa的代码。
...
其中代码中涉及的block_sparse_attn函数由下面这段代码给出
```python
import torch
...
现在，我为你提供两份参考脚本，分别是meta开源的triton版本HSTU与开源的triton版本native sparse attention实现

这是meta开源的triton版本HSTU实现
```python
...
这是开源的triton版本native sparse attention实现
```python
# -*- coding: utf-8 -*-
...
我这里有一份triton实现的代码。其中，代码的g_score是在类内部计算的，我希望改成将门控分数直接传参进来，并尽可能在计算中保持jagged结构
