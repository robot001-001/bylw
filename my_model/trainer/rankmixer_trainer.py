...
from model.rankmixer_embedding import RankMixerEmb
from model.rankmixer_sim import SIM
...
class RANKMIXERTrainer:
    ...
    def define_flags(self):
        # setup params
        flags.DEFINE_string('model', 'RANKMIXER', 'model file')
    ...
    def run(self):
        ...
        if self.FLAGS.model == 'RANKMIXER':
            from model.RANKMIXER import RANKMIXER
            self.model_cls = RANKMIXER
    ...
    def get_model(self):
        ...
        self.input_preprocess_module = SIM(**self.model_args)
        self.input_preprocess_module.to(self.device)
        total_params = sum(p.numel() for p in self.input_preprocess_module.parameters())
        trainable_params = sum(p.numel() for p in self.input_preprocess_module.parameters() if p.requires_grad)

        logging.info(f"input_preprocess_module Total Parameters: {total_params}")
        logging.info(f"input_preprocess_module Trainable Parameters: {trainable_params}")

        self.embedding_module = RankMixerEmb(
    ...
    def get_loss(self):
        ...
        model_params = list(self.model.parameters()) + list(self.input_preprocess_module.parameters()) + list(self.embedding_module.parameters())
    ...
    def get_dataset(self):
        if self.FLAGS.dataset_name == 'ml-1m':
            ...
            self.dataset = MovieLensFullDataset(
                # f'tmp/ml-1m/sasrec_format_binary_augment_onetrans_v1.csv',
            ...
            self.test_dataset = MovieLensFullDataset(
                # f'tmp/ml-1m/sasrec_format_binary_onetrans_testset_v1.csv',
        ...
        elif self.FLAGS.dataset_name == 'amzn-books':
            logging.info(f'getting train set')
            self.dataset = MovieLensFullDataset(
                f'tmp/amzn-books/sasrec_format_binary_augment_onetrans_v1.csv',
                # f'tmp/amzn-books/sasrec_format_binary_augment_onetrans.csv',
                max_len=self.FLAGS.max_seq_len
            )
            logging.info(f'dataset.max_item_id: {self.dataset.max_item_id}')
            self.train_data_sampler, self.train_data_loader = create_data_loader(
            ...
            logging.info(f'getting test set')
            self.test_dataset = MovieLensFullDataset(
                f'tmp/amzn-books/sasrec_format_binary_onetrans_testset_v1.csv',
                # f'tmp/amzn-books/sasrec_format_binary_onetrans_testset.csv',
                max_len=self.FLAGS.max_seq_len
            )
            self.max_item_id = self.test_dataset.max_item_id
            self.max_user_id = self.test_dataset.max_user_id
