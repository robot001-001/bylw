...
def naive_nsa(
    ...
    for i in range(len(cu_seqlens) - 1):
        ...
        for i_q in range(T):
            # 检查是否为奇数位置（sx1），如果是则复用前一个偶数位置（sx0）的激活结果
            if i_q % 2 == 1:
                # 奇数位置：使用前一个偶数位置的激活结果
                prev_i_q = i_q - 1
                if not varlen:
                    o_slc[i, i_q] = o_slc[i, prev_i_q]
                    if window_size > 0 and o_swa is not None:
                        o_swa[i, i_q] = o_swa[i, prev_i_q]
                else:
                    o_slc[0][cu_seqlens[i]+i_q] = o_slc[0][cu_seqlens[i]+prev_i_q]
                    if window_size > 0 and o_swa is not None:
                        o_swa[0][cu_seqlens[i]+i_q] = o_swa[0][cu_seqlens[i]+prev_i_q]
                continue
...
def naive_nsa_compression_varlen(
    ...
    for i in range(len(cu_seqlens) - 1):
        ...
        o_cmp_b = torch.einsum('hqk,khd->qhd', attn_cmp, v_cmp).nan_to_num() * g_cmp_b.unsqueeze(-1)
        attn_select = attn_cmp.masked_fill(local_mask, float(1.0))
        ...
        block_indices_b = block_indices_b.transpose(0, 1)
        
        # 对于奇数位置（sx1），使用前一个偶数位置（sx0）的block_indices和激活结果
        for t_b in range(1, T_b, 2):
            prev_t_b = t_b - 1
            block_indices_b[t_b, :, :S_b] = block_indices_b[prev_t_b, :, :S_b]
            o_cmp_b[t_b] = o_cmp_b[prev_t_b]
        
        block_indices[0][cu_seqlens[i]:cu_seqlens[i+1], :, :S_b] = block_indices_b
        o_cmp[0][cu_seqlens[i]:cu_seqlens[i+1]] = o_cmp_b
